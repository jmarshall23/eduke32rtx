...
void animvpx_setup_glstate(void)
{
    ...
    if (glinfo.glsl)
    {
        ...
        /* Compile the shader. */
        glShaderSourceARB(FSHandle, 1, (const GLcharARB **)&fragprog_src, NULL);
        glCompileShaderARB(FSHandle);

        /* Print the compilation log. */
        glGetObjectParameterivARB(FSHandle, GL_OBJECT_COMPILE_STATUS_ARB, &gli);
        glGetInfoLogARB(FSHandle, sizeof(logbuf), NULL, logbuf);
        if (logbuf[0])
        ...
        glGetInfoLogARB(PHandle, sizeof(logbuf), NULL, logbuf);
        if (logbuf[0])
